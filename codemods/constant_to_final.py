...
Usage:
```bash
python libcst_add_final.py <path>
```
"""
...
def _build_final_annotation(
    ...
    value = getattr(existing_annotation.annotation, "value", "")
    if value:
        value_value = getattr(value, "value", "")
        if value in _EXCLUDE_ANNOTATIONS or value_value in _EXCLUDE_ANNOTATIONS:
        ...
        annotation=cst.Subscript(
            ...
            slice=[
                cst.SubscriptElement(
                    slice=cst.Index(value=existing_annotation.annotation)
                ),
            ],
        )
    )
...
class FinalTransformer(cst.CSTTransformer):
    ...
    def leave_Module(
        ...
        if self.module_needs_final and not self.final_imported:
    ...
    def leave_Assign(
        ...
        if (
            ...
            and original_node.targets[0].target.value.isupper()
        ):
    ...
    def leave_AnnAssign(
        ...
        if (
            ...
            and original_node.target.value.isupper()
        ):
...
def _update_module(module: pathlib.Path, include_class_defs: bool) -> None:
    ...
    try:
        ...
        modified_tree = tree.visit(
            FinalTransformer(include_class_assigns=include_class_defs)
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Add Final annotations to all uppercase variables in Python files."
    )
    parser.add_argument(
        "path",
        type=str,
